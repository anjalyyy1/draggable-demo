...
export default class Rect extends PureComponent {
  ...
  render () {
    ...
    const style = {
      ...
      height: Math.abs(height) || "auto",
      width: Math.abs(width),
      left: centerX - (Math.abs(width) / 2),
      top: centerY - ((Math.abs(height) || 0) / 2),
    }
    ...
    return (
      ...
      >
        {
          borderDirections.map(direction => (<div className={"border "+direction} style={borderStyle} key={direction} onMouseDown={this.startDrag} onTouchStart={this.startDrag} />))
        }

        {
          ...
          <div className="rotate" onMouseDown={this.startRotate} onTouchStart={this.startRotate}>
            ...
            {
              isRotating && <span
                  className={"rotate-angle-display"}
                  style={{
                    transform: `rotate(-${rotateAngle}deg) translateY(${rotateAngle <= 90 || rotateAngle >= 270 ? "-" : ""}25px) translateX(${rotateAngle <= 180 ? "" : "-"}25px)`,
                  //   left: (styling.left + (styling.width / 2))+"px"
                  }}
              >{rotateAngle}</span>
            }
          </div>
